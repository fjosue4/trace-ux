...
func (s *Server) handleDemoClaim(w http.ResponseWriter, r *http.Request) {
	...
	s.initSecurity()
	if !s.demoClaimLimiter.allow("ip:" + s.clientIP(r)) {
		writeRateLimited(w, "too many demo replay requests")
		return
	}
...
func (s *Server) demoSession(token string) (*Session, bool) {
	...
	if err != nil || sess == nil || sess.SiteID != rec.SiteID {
		return nil, false
	}
	return sess, true
}
...
func (s *Server) handleDemoReplay(w http.ResponseWriter, r *http.Request) {
	...
	activity, err := s.store.GetCustomEvents(sess.ID)
	if err != nil {
		writeErr(w, http.StatusInternalServerError, "could not load replay actions")
		return
	}
	logs, err := s.store.GetSessionLogs(sess.ID)
	if err != nil {
		writeErr(w, http.StatusInternalServerError, "could not load replay actions")
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{
		"session":       publicDemoSession(sess),
		"custom_events": activity,
		"logs":          publicDemoLogs(logs),
	})
}
...
func publicDemoLogs(logs []Log) []map[string]any {
	...
	for _, item := range logs {
		out = append(out, map[string]any{
			"id": item.ID, "timestamp_ms": item.TimestampMs, "severity": item.Severity,
			"message": item.Message, "url": item.URL,
		})
	}
	return out
}
