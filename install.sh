...
Environment overrides:
  TRACE_UX_PASSWORD=...   admin password (a random one is generated if unset)
...
PORT="$(grep '^TRACE_UX_ADDR=' "$ENV_FILE" | cut -d= -f2 | tr -d ':')"
IP="$(hostname -I 2>/dev/null | awk '{print $1}')"
echo "  dashboard : http://${IP:-localhost}:${PORT}/"
if [[ -f "$ENV_FILE" ]]; then
  echo "  login     : admin / $(grep '^TRACE_UX_PASSWORD=' "$ENV_FILE" | cut -d= -f2)"
fi
